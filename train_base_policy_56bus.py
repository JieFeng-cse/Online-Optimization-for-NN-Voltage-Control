...
from env.env_single_phase_56bus import VoltageCtrl_nonlinear, create_56bus
...
def get_args():
    ...
    parser.add_argument("--vlr", type=float, default=2e-4, help="Value network learning rate")
...
if (FLAG ==0): 
    # load trained policy
    for i in range(n_control):
        valuenet_dict = torch.load(f'checkpoints/single/{env_name}/linear_control/value_net_checkpoint_a{i}.pth')
        policynet_dict = torch.load(f'checkpoints/single/{env_name}/linear_control/policy_net_checkpoint_a{i}.pth')
        agent_list[i].value_net.load_state_dict(valuenet_dict)
        agent_list[i].policy_net.load_state_dict(policynet_dict)

elif (FLAG ==1):
    num_episodes = 1500 #1500
    ...
    for episode in range(num_episodes):
        ...
        for step in range(num_steps):
            ...
            for i in range(n_control):
                ...
                action_agent = agent_list[i].policy_net.get_action(np.square(np.asarray([state[i]]))) \
                    + np.random.normal(0, max_ac)/(episode+1) ** 0.3
                action_p.append(action_agent)                    
                action.append(action_agent)
                # grad=agent_list[i].policy_net.get_param_grad(np.square(np.asarray([state[control_id[i]]])))
            ...
            if(np.min(next_state)<0.75): #if voltage violation > 25%, episode ends.
                ...
                for i in range(n_control): 
                    state_buffer = (np.square(np.asarray([state[i]]))).reshape(ph_num,) 
                    action_buffer = action[i].reshape(ph_num,)
                    last_action_buffer = last_action[i].reshape(ph_num,)
                    next_state_buffer = (np.square(np.asarray([next_state[i]]))).reshape(ph_num, )
...
plt.savefig(f'figures/{env_name}_controllers_{topo_name}.png', dpi=300, bbox_inches='tight')
plt.show()
...
for i in range(n_control):
    axs[i].plot(range(len(state_list)), np.array(state_list)[:,i], '-.', label = 'states')
    axs[i].legend(loc='lower left')
